...
# Manifeste : Tetram.Media.Similarity.psd1 — Test-ModuleManifest puis Import-Module (Join-Path $RepoRoot 'Tetram.Media.Similarity.psd1') -Force
...
Describe 'Test-MediaSimilarity - résolution FFmpeg au démarrage' {
    ...
    It "log une erreur via Write-ErrorLog et ne lève pas d'exception quand FFmpeg est introuvable" {
        { Test-MediaSimilarity -Path $script:RepoRootSimilarity } | Should -Not -Throw
