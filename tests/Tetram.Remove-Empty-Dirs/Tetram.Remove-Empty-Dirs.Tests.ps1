...
# Import-Module (Join-Path $RepoRoot 'Tetram.Remove-Empty-Dirs') -Force après éventuelle étape Test-ModuleManifest sur le même .psd1
...
BeforeAll {
    ...
    $script:ModuleRootEmptyDirs = Join-Path $script:RepoRootEmptyDirs 'Tetram.Remove-Empty-Dirs'
    $script:ManifestEmptyDirs = Join-Path $script:ModuleRootEmptyDirs 'Tetram.Remove-Empty-Dirs.psd1'
    Import-Module -Name $script:ModuleRootEmptyDirs -Force -ErrorAction Stop
}
...
Describe 'Remove-EmptyDirs -DeepScan -WhatIf' {
    ...
    It 'termine quand un dossier vide existe (WhatIf ne compte pas comme suppression)' {
        ...
        } -ArgumentList $script:ModuleRootEmptyDirs, $TestDrive.ToString()
