# Étendre la suite autour du module SUD Tetram.Media.VideoUtils/Tetram.Media.VideoUtils.psd1.
#
# RepoRoot depuis tests/<Module> (deux `..`) : $RepoRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..' '..')).Path
# Import-Module (Join-Path $RepoRoot 'Tetram.Media.VideoUtils') ; pour comportements combinés FFmpeg, importer Tetram.Media.FFmpeg en amont avec Mock sur exécution.
...
BeforeAll {
    ...
    Import-Module -Name (Join-Path $script:RepoRootVideoUtils 'Tetram.Media.VideoUtils') -Force -ErrorAction Stop
}
