...
# Manifeste : Tetram.Media.Streams/Tetram.Media.Streams.psd1 — Test-ModuleManifest puis Import-Module -Force
...
BeforeAll {
    ...
    $script:ModuleRootStreams = Join-Path $script:RepoRootStreams 'Tetram.Media.Streams'
    $script:ManifestStreams = Join-Path $script:ModuleRootStreams 'Tetram.Media.Streams.psd1'
}
...
Describe 'Tetram.Media.Streams exports' {
    BeforeAll {
        Import-Module -Name $script:ModuleRootStreams -Force -ErrorAction Stop
    }
    AfterAll {
