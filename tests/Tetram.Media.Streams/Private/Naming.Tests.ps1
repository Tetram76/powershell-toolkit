...
# Import-Module (Join-Path $RepoRoot 'Tetram.Media.Streams') ; InModuleScope 'Tetram.Media.Streams' { … }
...
Describe 'ConvertTo / ConvertFrom-StreamFileName' {
    ...
    It 'peluche n/flags/langue depuis la fin ; un jeton restant n''est pas un fichier de flux du MKV' {
        InModuleScope 'Tetram.Media.Streams' {
            ...
            $ok.Language | Should -Be 'eng'
            $ok.Flags | Should -Be @('forced', 'commentary')
            $ok.CollisionIndex | Should -Be 2
            ConvertFrom-StreamFileName -Basename 'film' -FileName 'film.2.eng.srt' | Should -BeNullOrEmpty
