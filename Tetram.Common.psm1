...
function Format-FileSize {
    ...
    if ($Size -lt 0) { return "-$(Format-FileSize -Size (-$Size))" }
    if ($Size -gt 1PB) { return "{0:0.00} PB" -f ($Size/1PB) }
    if ($Size -gt 1TB) { return "{0:0.00} TB" -f ($Size/1TB) }
    if ($Size -gt 1GB) { return "{0:0.00} GB" -f ($Size/1GB) }
    if ($Size -gt 1MB) { return "{0:0.00} MB" -f ($Size/1MB) }
    if ($Size -gt 1KB) { return "{0:0.00} kB" -f ($Size/1KB) }
...
function Format-Duration {
    ...
    if (-not $TimeSpan) { return "" }
    if ($TimeSpan.Days -eq 0) { return $TimeSpan.ToString("h':'mm':'ss") }
...
function Test-IsLikelyPath {
    ...
    if ($trimmed -match '^[a-zA-Z][a-zA-Z0-9+\-.]*://') {
        if ([System.Uri]::IsWellFormedUriString($trimmed, [System.UriKind]::Absolute)) { return $true }
    }
    ...
    if ($trimmed.IndexOfAny($InvalidChars) -ge 0) { return $false }
    ...
    if ($trimmed -match '\.(?<extension>[a-zA-Z0-9]+)$') {
        if ($trimmed -match '^\d+\.\d+$') { return $false } # Exclure nombres
        if ($Matches['extension'].Length -le $MaxExtensionLength) { return $true }
    }
