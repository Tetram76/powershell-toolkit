...
function Get-TargetAudioBitrate {
    ...
    if ($Codec -eq 'opus') {
        if ($isStereo) { return @{ High='160k'; Medium='128k'; Low='96k' }[$Quality] }
        if ($is51)     { return @{ High='384k'; Medium='320k'; Low='224k' }[$Quality] }
        if ($is71)     { return @{ High='512k'; Medium='384k'; Low='320k' }[$Quality] }
        return @{ High='320k'; Medium='256k'; Low='192k' }[$Quality]
    }

    # aac
    if ($isStereo) { return @{ High='192k'; Medium='160k'; Low='128k' }[$Quality] }
    if ($is51)     { return @{ High='448k'; Medium='384k'; Low='320k' }[$Quality] }
    if ($is71)     { return @{ High='576k'; Medium='448k'; Low='384k' }[$Quality] }
...
function ConvertTo-IntBitrateK {
    ...
    if ($BitrateK -match '^(\d+)\s*k$') { return [int]$Matches[1] * 1000 }
    if ($BitrateK -match '^\d+$') { return [int]$BitrateK } # au cas où
    return 0
}
...
function Test-HasBitrateGain {
    ...
    if ($SourceBitrate -le 0) { return $false }

    $targetBps = ConvertTo-IntBitrateK $TargetBitrateLabel
    if ($targetBps -le 0) { return $false }
