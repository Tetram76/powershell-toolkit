...
function Get-SourceChromaMode {
    ...
    if ([string]::IsNullOrWhiteSpace($pf)) { return '420' } # fallback

    # Beaucoup de formats 420 apparaissent sous nv12/p010le etc.
    if ($pf -match '^(nv12|p010|p016)') { return '420' }

    # yuv420*, yuv422*, yuv444*
    if ($pf -match '^yuv420') { return '420' }
    if ($pf -match '^yuv422') { return '422' }
    if ($pf -match '^yuv444') { return '444' }

    # Cas courants RGB planar (ex: gbrp/gbrp10le) -> assimilé 444
    if ($pf -match '^gbrp') { return '444' }
