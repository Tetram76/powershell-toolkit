...
Importer `.\Tetram.Media.Streams` (PowerShell 7.6+). `-MediaFile` est un fichier `.mkv` existant, accepte le pipeline (chaîne simple). Le MKV source n'est **jamais modifié** : la commande ne fait que lire (`ffprobe`) et copier des flux vers des fichiers de flux (`ffmpeg -c copy`), d'où le verbe `Get` plutôt que `Split`.
...
Noms : `{basename}.{langue}[.default][.forced][.commentary][.original][.dub][.hearing_impaired][.visual_impaired][.{n}].{ext}` pour vidéo, audio et sous-titres. `basename` vient du MKV. Lecture depuis la fin ; un jeton restant = pas un fichier de flux. Langue absente, `und` ou `unk` : jeton `und` (ISO 639-2 indéterminée). Un sidecar sans jeton langue se parse comme `und`. `dub` est un flag, pas une langue. Uniquement vidéo, audio et sous-titres extraits : covers, polices et chapitres restent dans le MKV.
...
Prérequis : PowerShell 7.6+, ffmpeg/ffprobe (`Tetram.Media.FFmpeg`). Importer `.\Tetram.Media.Streams`. Copie bit-exacte (`-c copy`), pas de réencodage. Le MKV source n'est jamais modifié.
