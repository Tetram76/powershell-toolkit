...
Sans `-OutputPath`, le fichier final est écrit à côté de la source, avec le suffixe `.fr` avant l'extension (`episode.ass` → `episode.fr.ass`). Si le fichier final existe déjà, la commande échoue sans écraser et sans appeler le modèle. Un ancien `.raw.json` à côté de la sortie n'est ni bloquant, ni modifié.
...
ConvertTo-FrenchSubtitle `
    ...
    -SecondarySourcePath @(
        ...
        'D:\Media\episode.track 1.ja.large-v3.json',
...
0..N sources linguistiques facultatives : sous-titres SRT/ASS/SSA ou transcriptions JSON Tetram canoniques (typiquement produites par `Get-MediaTranscript`, ex. `Episode.track 1.ja.large-v3.json`). L'ordre fourni ne représente aucune priorité linguistique.
...
Prérequis : PowerShell 7+. Les chemins `-SubtitlePath` et `-SecondarySourcePath` sont littéraux (pas de jokers). `-SubtitlePath` est obligatoire. `-SecondarySourcePath` accepte 0..N fichiers. La commande lève une exception si la sortie existe déjà, ou si la réponse du modèle est inutilisable au niveau transport : dans ces cas aucun fichier final n'est écrit. Un échec de reconstruction (JSON incomplet ou invalide, contrat ASS `{...}` / `\N`) n'écrit pas de final et émet un warning.
