...
- réencodage / `-NoTranscode` : ffmpeg écrit un temporaire sous `-TempPath`, puis `Move-Item` écrase le fichier source, puis un `Rename-Item` change l'extension si besoin (réencodage vers `.mkv`). Les horodatages du fichier sont restaurés. Des dossiers voisins peuvent voir leurs dates corrigées via NFO (`premiered`). En réencodage, un contrôle temporel par fenêtres ffprobe bornées (`pts_time`) vérifie l'étendue des flux audio/vidéo conservés et leurs offsets relatifs. Le contrôle d'entrelacement (`pos`) peut effectuer une lecture ciblée supplémentaire lorsqu'un stream actif n'est pas retrouvé dans la fenêtre commune, afin de détecter un stream physiquement retardé. Une troncature ou un décalage A/V significatif, un fichier de sortie que ffprobe ne peut pas sonder, un flux A/V mappé absent, ou un écart physique d'entrelacement trop large rejettent la sortie et conservent l'original par défaut. Les sous-titres sont exclus des contrôles temporels. Avec `-AllowIntegrityMismatch`, le même mismatch reste détecté et signalé, mais devient un avertissement non bloquant et la sortie est acceptée. Un résultat `unknown` (données insuffisantes pour placer une fenêtre bornée) est accepté avec avertissement, sans exiger le commutateur. `-NoTranscode` n'exécute pas ce contrôle.

Récapitulatif de fin de session : après le compteur `N file(s) accepted with integrity warning:` (N = nombre de fichiers distincts concernés), chaque fichier apparaît sur sa propre ligne, suivi en retrait du ou des avertissements d'intégrité qu'il a réellement produits (mismatch toléré par `-AllowIntegrityMismatch` ou résultat `unknown`), avec leur message complet tel qu'il a été émis pendant le traitement. Pour un même fichier, un message strictement identique répété n'est affiché qu'une fois, suffixé de ` [x N]` (N = nombre d'occurrences) ; une occurrence unique n'a pas de suffixe. Ce regroupement ne porte que sur les répétitions au sein d'un même fichier : deux fichiers différents ne sont jamais fusionnés sur une même ligne, même avec un message identique. Deux avertissements du même type dont les valeurs diffèrent (flux, durées, positions...) restent deux lignes distinctes sous le même fichier. Les fichiers, puis les messages au sein de chacun, suivent l'ordre de première apparition. Le récapitulatif des fichiers rejetés est inchangé.
