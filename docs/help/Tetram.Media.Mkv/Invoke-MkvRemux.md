...
origine fichier commune (min(pts) des flux mappés ancrables des deux côtés, via chaque
`time_base`) : `packet-end-span` = max(pts + duration) − origine si la source
et la sortie ont cette borne exacte ; `packet-pts-span` = max(pts) − origine
si la source n'a pas de borne exacte mais a une étendue PTS des deux côtés ;
mismatch si la source avait une borne exacte que la sortie a perdue.
...
PGS : le contrôle utilise la métrique PTS et ne dépend pas de `packet.duration`.
...
(`unknown`) était déjà accepté et n'est pas la raison d'être de ce paramètre.
