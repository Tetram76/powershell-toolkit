...
Récapitulatif de fin de session : après le compteur `N file(s) accepted with integrity warning:`, chaque avertissement d'intégrité accepté (mismatch toléré par `-AllowIntegrityMismatch` ou résultat `unknown`) est rappelé avec son message complet, tel qu'il a été émis pendant le traitement. Un message strictement identique répété n'est affiché qu'une fois, suffixé de ` [x N]` (N = nombre d'occurrences) ; une occurrence unique n'a pas de suffixe. Deux avertissements du même type dont les valeurs diffèrent (flux, durées, positions, fichier...) restent deux lignes distinctes. Les lignes suivent l'ordre de première apparition. Le récapitulatif des fichiers rejetés est inchangé.
